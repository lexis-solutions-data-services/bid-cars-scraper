...
async function runActor() {
  ...
  const actorId = `lexis-solutions/bid-cars-scraper`;
  console.log(`Starting actor: ${actorId}`);

  const { defaultDatasetId } = await client.actor(actorId).call({
    startUrls: [
      {
        "url": "https://bid.cars/en/search/results?search-type=filters&status=All&type=Automobile&make=All&model=All&year-from=1900&year-to=2026&auction-type=All&exterior-color=All"
      }
    ],
    maxItems: 100,
    proxyConfiguration: {
